...
window.AppKlinikPasien = {
    ...
    init: function() {
        window.sb.from('pasien').select('*').order('nama', { ascending: true }).then(function(snap) { if(snap.error) throw snap.error;
            AppKlinikPasien.data = [];
            (snap.data || []).forEach(function(doc) { var d = doc; d.id = doc.id; AppKlinikPasien.data.push(d); });
    ...
    executeImport: function() {
        ...
        var _batchOps = [];
            chunk.forEach(pasien => {
                ...
                var docRef = safeRM ? window.sb.from('pasien').select('*').doc(safeRM) : window.sb.from('pasien').select('*').doc();
                batch.set(docRef, {
