...
window.AppKlinikRekamMedis = {
    ...
    init: function() {
        ...
        pTindakan.then(function(snap) { if(snap.error) throw snap.error;
            AppKlinikRekamMedis.tindakanList = [];
            (snap.data || []).forEach(function(doc) { var d = doc; d.id = doc.id; AppKlinikRekamMedis.tindakanList.push(d); });
                ...
                window.sb.from('antrian').select('*').eq('id', antId).single().then(doc => {
                    if (doc.exists) {
                        ...
                        if (a.status === 'menunggu') {
                            if(window.TEMP_ANTRIAN_ID) window.sb.from('antrian').update({ status: 'dilayani', waktuMulai: new Date().toISOString() }).eq('id', window.TEMP_ANTRIAN_ID);
                        }
