...
window.AppKlinikResep = {
    ...
    init: function() {
        ...
        window.sb.from('rekam_medis').select('*')
            ...
            .limit(200)
            .then(function(snap) { if(snap.error) throw snap.error;
            AppKlinikResep.data = [];
            (snap.data || []).forEach(function(doc) { 
                var d = doc; 
                d.id = doc.id; 
    ...
    renderList: function() {
        ...
        var listResep = this.data.filter(function(rm) {
            return (rm.catatan && rm.catatan.trim() !== '') || (rm.tindakan_items && rm.tindakan_items.length > 0);
        });
        ...
        listResep.forEach(function(rm) {
            ...
            html += '<td class="px-4 py-3 font-medium text-gray-800 dark:text-white">' + Utils.escapeHtml(rm.nama_pasien || '-') + '<br><span class="text-xs text-slate-400 font-mono">' + Utils.escapeHtml(rm.nomor_rm || '-') + '</span></td>';
    ...
    showDetail: function(rmId) {
        ...
        html += '<div><p class="text-xs text-slate-400">Pasien</p><p class="font-semibold text-gray-800 dark:text-white">' + Utils.escapeHtml(rm.nama_pasien || '-') + '</p><p class="text-xs text-slate-500">' + Utils.escapeHtml(rm.nomor_rm || '-') + '</p></div>';
        ...
        if (rm.tindakan_items && rm.tindakan_items.length > 0) {
            ...
            rm.tindakan_items.forEach(function(t) {
                html += '<div class="flex justify-between text-xs"><span class="text-gray-700 dark:text-slate-200">' + Utils.escapeHtml(t.namaTindakan) + '</span><span class="font-semibold text-purple-600">' + Utils.formatRupiah(t.harga_jual) + '</span></div>';
            });
            html += '</div></div>';
        }
