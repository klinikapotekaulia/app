...
window.AppPengaturanPembagian = {
    ...
    init: function() {
        ...
        Promise.all([pKaryawan, pConfig]).then(function(results) {
            self.karyawanList = [];
            results[0].data.forEach(function(doc) { var d = doc; d.id = doc.id; self.karyawanList.push(d); });
            ...
            if (results[1].exists) {
                ...
                if (!Array.isArray(self.data.resep_klinik)) self.data.resep_klinik = [];
    ...
    getDefaultData: function() {
        return {
            resep_klinik: [],
            resep_luar: { nilai_resep: 0, potonganDokter: 0 },
            marginResep: 35,
            tindakanKlinik: [],
            tindakanApotek: [],
    ...
    renderForm: function() {
        ...
        if(d.resep_klinik.length === 0) html += '<p class="text-sm text-slate-400 italic p-2">Belum ada skema. Klik tambah untuk buat skema per dokter.</p>';
        ...
        html += this.inputField('Nilai Resep', 'resepLuar_nilaiResep', d.resep_luar.nilai_resep, 'Rp');
    ...
    addResepKlinik: function() {
        ...
        this.data.resep_klinik.push({ dokter_id: '', nilai_resep: 0, jm: 0, jd: 0, poolKaryKlinik: 0, slotKaryKlinik: [], poolKaryApotek: 0, slotKaryApotek: [] });
        this.renderForm();
    },
    ...
    removeResepKlinik: function(index) {
        if(confirm('Hapus skema dokter ini?')) {
            this.syncStateFromDOM();
            this.data.resep_klinik.splice(index, 1);
            this.renderForm();
        }
    },
    ...
    renderResepKlinikBlock: function(index, doc) {
        ...
        html += '<div class="flex items-center gap-2">' + this.inputField('Pool Total', 'rk-poolKlinik-' + index, doc.poolKaryKlinik, 'Rp') + '<button type="button" onclick="AppPengaturanPembagian.addSlotTo(\'resep_klinik\', ' + index + ', \'slotKaryKlinik\')" class="text-xs text-purple-600 px-2 py-2 font-medium">+ Kary</button></div></div>';
        ...
        html += '<div class="flex items-center gap-2">' + this.inputField('Pool Total', 'rk-poolApotek-' + index, doc.poolKaryApotek, 'Rp') + '<button type="button" onclick="AppPengaturanPembagian.addSlotTo(\'resep_klinik\', ' + index + ', \'slotKaryApotek\')" class="text-xs text-teal-600 px-2 py-2 font-medium">+ Kary</button></div></div>';
    ...
    renderSlotRow: function(rowId, slot, hasThr) {
        ...
        html += '<div class="w-full sm:w-1/3">' + this.dropdownKaryawan('slot-id-' + rowId, slot.karyawan_id) + '</div>';
    ...
    addSlotTo: function(parentKey, docIndex, slotKey) {
        this.syncStateFromDOM(); 
        var newSlot = { karyawan_id: '', persen: 0, isTHR: false };
        
        if (parentKey === 'resep_klinik') {
            this.data.resep_klinik[docIndex][slotKey].push(newSlot);
        } else {
    ...
    simpan: function() {
        ...
        var globalRef = window.sb.from('pengaturan_pembagian').select('*').doc('global');
        batch.set(globalRef, d, { merge: true });

        // 2. Buat Snapshot History
        var historyRef = window.sb.from('pengaturan_pembagian_history').select('*').doc();
