...
window.AppPengaturanPembagian = {
    ...
    init: function() {
        ...
        var pConfig = window.sb.from('pengaturanPembagian').select('*').eq('id', 'global').single();
    ...
    simpan: function() {
        ...
        var globalRef = window.sb.from('pengaturanPembagian').doc('global');
        batch.set(globalRef, d, { merge: true });

        // 2. Buat Snapshot History
        var historyRef = window.sb.from('pengaturanPembagianHistory').doc();
