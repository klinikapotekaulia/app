...
window.AppPengaturanUsers = {
    ...
    init: function() {
        ...
        window.sb.from('users').order('nama', { ascending: true }).get().then(snap => {
            AppPengaturanUsers.data = [];
            (data || []).forEach(doc => { var d = doc; d.id = doc.id; AppPengaturanUsers.data.push(d); });
    ...
    toggleStatus: function(id, currentStatus) {
        ...
        window.sb.from('users').update({ status: newStatus })
            .then(() => {
