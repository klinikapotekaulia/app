...
window.AppPengaturanUsers = {
    ...
    init: function() {
        ...
        window.sb.from('users').select('*').order('nama', { ascending: true }).then(function(snap) { if(snap.error) throw snap.error;
            AppPengaturanUsers.data = [];
            (snap.data || []).forEach(function(doc) { var d = doc; d.id = doc.id; AppPengaturanUsers.data.push(d); });
    ...
    renderList: function() {
        ...
        this.data.forEach(u => {
            ...
            if (({ uid: window.currentUserId }) && u.id !== window.currentUserId) { // FIX: null-safe
    ...
    simpan: function() {
        ...
        if (isEdit) {
            ...
            var editId = document.getElementById('fu-id').value;
            window.sb.from('users').update({
                nama: nama, role: role, status: status, updatedAt: new Date().toISOString()
            }).eq('id', editId).then(({error})=> { if(error) throw error; => {
            ...
            ready.then(function() {
                ...
                secondaryApp.auth().createUserWithEmailAndPassword(email, pass)
                    .then(function(userCredential) {
                        ...
                        return window.sb.from('users').upsert({
    ...
    toggleStatus: function(id, currentStatus) {
        ...
        window.sb.from('users').update({ status: newStatus }).eq('id', id).then(({error})=> { if(error) throw error; => {
