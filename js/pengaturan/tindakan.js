...
window.AppPengaturanTindakan = {
    ...
    render: function() {
        ...
        window.sb.from('masterTindakan').order('nama', { ascending: true }).get().then(snap => {
        ...
        defaults.forEach(function(item) {
            var newDocRef = window.sb.from('masterTindakan').doc();
    ...
    simpan: function() {
        ...
        var p;
        if (isEdit) {
            p = window.sb.from('masterTindakan').update(obj);
        } else {
            obj.createdAt = new Date().toISOString();
            p = window.sb.from('masterTindakan').insert(obj);
        }
    ...
    hapus: function(id, nama) {
        ...
        window.sb.from('masterTindakan').delete().eq('id', id).then(() => {
