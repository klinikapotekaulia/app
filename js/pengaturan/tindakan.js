...
window.AppPengaturanTindakan = {
    ...
    render: function() {
        ...
        window.sb.from('master_tindakan').select('*').order('nama', { ascending: true }).then(function(snap) { if(snap.error) throw snap.error;
        ...
        defaults.forEach(function(item) {
            var newDocRef = window.sb.from('master_tindakan').select('*').doc();
