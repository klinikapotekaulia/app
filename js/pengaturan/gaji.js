...
window.AppPengaturanGaji = {
    ...
    init: function() {
        ...
        var pConfig = window.sb.from('pengaturanGaji').select('*').eq('id', 'global').single();
        ...
        Promise.all([pKaryawan, pConfig]).then(function(results) {
            ...
            if (results[1].exists) {
                ...
                window.sb.from('pengaturanGaji').upsert(AppPengaturanGaji.data);
            }
    ...
    simpan: function() {
        ...
        window.sb.from('pengaturanGaji').upsert(dataToSave, { merge: true })
