...
window.AppLaporanPiutang = {
    ...
    init: function() {
        ...
        var pPiutang = window.sb.from('piutang_karyawan').select('*').order('createdAt', { ascending: false }).limit(50);
    ...
    renderList: function() {
        ...
        this.data.forEach(function(p) {
            ...
            html += '<h3 class="font-bold text-gray-800 dark:text-white">' + Utils.escapeHtml(p.nama_karyawan || '-') + '</h3>';
    ...
    simpan: function() {
        ...
        if (!obj.karyawan_id || obj.jumlah <= 0) { Utils.toast('Lengkapi data', 'error'); return; }

        window.sb.from('piutang_karyawan').insert(obj).then(() => {
    ...
    lunasi: function(id) {
        ...
        window.sb.from('piutang_karyawan').update({
            status: 'lunas',
            tanggalLunas: new Date().toISOString().split('T')[0],
            lunasiOleh: window.currentUserName || 'Keuangan'
        }).eq('id', id).then(function(r) { if(r.error) throw r.error; return Promise.resolve(); }).then(() => {
