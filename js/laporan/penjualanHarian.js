 ...
 *   - transaksi { tanggal, tipe, totalBayar, items[], metode_pembayaran, jasa_resep }
 *
 * CARA PASANG:
...
window.AppLaporanPenjualanHarian = {
    ...
    init: function() {
        ...
        window.sb.from('transaksi').select('*')
            .gte('tanggal', dates.start)
            .lte('tanggal', dates.end)
            .order('tanggal', { ascending: true })
            
            .then(function(snap) { if(snap.error) throw snap.error;
                self.data = [];
                (snap.data || []).forEach(function(doc) { var d = doc; d.id = doc.id; self.data.push(d); });
    ...
    renderReport: function(dates) {
        ...
        this.data.forEach(function(t) {
            ...
            var met = (t.metode_pembayaran || 'cash').toLowerCase();
            ...
            if (t.items && Array.isArray(t.items)) {
                t.items.forEach(function(item) {
                    var nama = item.nama_obat || item.nama || '-';
                    if (!obatMap[nama]) obatMap[nama] = { qty: 0, omzet: 0 };
                    obatMap[nama].qty   += (item.jumlah || 0);
                    obatMap[nama].omzet += (item.jumlah || 0) * (item.harga_jual || 0);
                });
            }
        });
        ...
        this.data.slice().reverse().forEach(function(t) {
            ...
            html += '<td class="px-4 py-3 text-gray-800 dark:text-white">' + Utils.escapeHtml(t.nama_pasien || t.namaResep || t.keterangan || '-') + '</td>';
            html += '<td class="px-4 py-3 text-center capitalize text-slate-600 dark:text-slate-300">' + Utils.escapeHtml(t.metode_pembayaran || 'cash') + '</td>';
    ...
    exportCSV: function() {
        ...
        this.data.forEach(function(t) {
            rows.push([
                ...
                t.nama_pasien || t.namaResep || t.keterangan || '',
