...
window.AppLaporanPengeluaran = {
    ...
    init: function() {
        ...
        window.sb.from('kas_keluar').select('*').order('createdAt', { ascending: false }).limit(50).then(function(snap) { if(snap.error) throw snap.error;
            self.data = [];
    ...
    simpan: function() {
        ...
        window.sb.from('kas_keluar').insert(obj).then(() => {
    ...
    approve: function(id) {
        ...
        window.sb.from('kas_keluar').update({
            status: 'approved',
    ...
    reject: function(id) {
        ...
        window.sb.from('kas_keluar').update({ status: 'rejected' }).eq('id', id).then(function(r) { if(r.error) throw r.error;
