...
window.AppManajemenKaryawan = {
    ...
    init: function() {
        ...
        Promise.all([pKaryawan, pUsers]).then(function(results) {
            self.data = [];
            results[0].data.forEach(function(doc) { var d = doc; d.id = doc.id; self.data.push(d); });

            self.usersList = [];
            results[1].data.forEach(function(doc) { var d = doc; d.id = doc.id; self.usersList.push(d); });
    ...
    simpan: function() {
        ...
        p.then((docRef) => {
            ...
            if (user_id) {
                var userRef = window.sb.from('users').select('*').doc(user_id);
