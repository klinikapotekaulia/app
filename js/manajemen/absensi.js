...
window.AppManajemenAbsensi = {
    ...
    init: function() {
        ...
        Promise.all([pKaryawan, pAbsensi]).then(function(results) {
            ...
            self.data = [];
            results[1].data.forEach(function(doc) { var d = doc; d.id = doc.id; self.data.push(d); });
    ...
    renderMyAbsensi: function() {
        ...
        if (!myAbsen) {
            ...
            var jamMasuk = new Date(myAbsen.check_in).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
            ...
            // Sudah check-out
            var jamMasuk2 = new Date(myAbsen.check_in).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
            var jamPulang = new Date(myAbsen.check_out).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
