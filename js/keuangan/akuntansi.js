...
window.AppKeuanganAkuntansi = {
    ...
    init: function() {
        ...
        var pKasKeluar = window.sb.from('kas_keluar').eq('status', 'approved').gte('tanggal', startDate).lte('tanggal', endDate).get();
        var pBeliStok = window.sb.from('pembelianStok').gte('tanggal', startDate).lte('tanggal', endDate).get();
        var pGaji = window.sb.from('penggajian').eq('bulan', bulan).get();
        var pJurnalManual = window.sb.from('jurnal_manual').gte('tanggal', startDate).lte('tanggal', endDate).get();
        var pSaldoAwal = window.sb.from('saldo_awal').lte('periode', bulan).get();
    ...
    postingJurnalPenutup: function() {
        ...
        window.sb.from('jurnal_manual').eq('isJurnalPenutup', true).gte('tanggal', bulan + '-01').lte('tanggal', bulan + '-31').get().then(function(snap) {
            if (!snap.empty) {
            ...
            for (var k in self.COA) {
                if (self.COA[k].kategori === 'Pendapatan') {
                    ...
                    if (balK > 0) {
                        ...
                        var ref1 = window.sb.from('jurnal_manual').doc();
            ...
            for (var k2 in self.COA) {
                if (self.COA[k2].kategori === 'Beban') {
                    ...
                    if (balD > 0) {
                        totalBeban += balD;
                        var ref2 = window.sb.from('jurnal_manual').doc();
            ...
            if (labaBersih !== 0) {
                var ref3 = window.sb.from('jurnal_manual').doc();
            ...
            if (priveBal > 0) {
                var ref4 = window.sb.from('jurnal_manual').doc();
    ...
    simpanSaldoAwal: function() {
        ...
        for (var k in this.COA) {
            ...
            if (nominal > 0) {
                var ref = window.sb.from('saldo_awal').doc();
