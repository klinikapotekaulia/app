...
window.AppKeuanganAkuntansi = {
    ...
    init: function() {
        ...
        var pKasKeluar = window.sb.from('kasKeluar').eq('status', 'approved').gte('tanggal', startDate).lte('tanggal', endDate).get();
        ...
        var pJurnalManual = window.sb.from('jurnalManual').gte('tanggal', startDate).lte('tanggal', endDate).get();
        var pSaldoAwal = window.sb.from('saldoAwal').lte('periode', bulan).get();
    ...
    simpanJurnal: function() {
        ...
        window.sb.from('jurnalManual').insert(obj).then(function() {
    ...
    hapusJurnal: function(id) {
        ...
        window.sb.from('jurnalManual').delete().eq('id', id).then(function() {
    ...
    postingJurnalPenutup: function() {
        ...
        window.sb.from('jurnalManual').eq('isJurnalPenutup', true).gte('tanggal', bulan + '-01').lte('tanggal', bulan + '-31').get().then(function(snap) {
            if (!snap.empty) {
            ...
            for (var k in self.COA) {
                if (self.COA[k].kategori === 'Pendapatan') {
                    ...
                    if (balK > 0) {
                        ...
                        var ref1 = window.sb.from('jurnalManual').doc();
                        batch.set(ref1, { tanggal: tanggalAkhir, keterangan: 'Tutup Pendapatan ' + self.COA[k].nama, akunDebit: k, akunKredit: '3-9000', debit: balK, kredit: balK, isJurnalPenutup: true, createdAt: new Date().toISOString() });
                    }
                }
            }
            ...
            for (var k2 in self.COA) {
                if (self.COA[k2].kategori === 'Beban') {
                    ...
                    if (balD > 0) {
                        totalBeban += balD;
                        var ref2 = window.sb.from('jurnalManual').doc();
                        batch.set(ref2, { tanggal: tanggalAkhir, keterangan: 'Tutup Beban ' + self.COA[k2].nama, akunDebit: '3-9000', akunKredit: k2, debit: balD, kredit: balD, isJurnalPenutup: true, createdAt: new Date().toISOString() });
                    }
                }
            }
            ...
            if (labaBersih !== 0) {
                var ref3 = window.sb.from('jurnalManual').doc();
                if (labaBersih > 0) { // Untung
                    batch.set(ref3, { tanggal: tanggalAkhir, keterangan: 'Tutup Ikhtisar L/R (Laba)', akunDebit: '3-9000', akunKredit: '3-3000', debit: labaBersih, kredit: labaBersih, isJurnalPenutup: true, createdAt: new Date().toISOString() });
                } else { // Rugi
                    batch.set(ref3, { tanggal: tanggalAkhir, keterangan: 'Tutup Ikhtisar L/R (Rugi)', akunDebit: '3-3000', akunKredit: '3-9000', debit: Math.abs(labaBersih), kredit: Math.abs(labaBersih), isJurnalPenutup: true, createdAt: new Date().toISOString() });
                }
            }
            ...
            if (priveBal > 0) {
                var ref4 = window.sb.from('jurnalManual').doc();
                batch.set(ref4, { tanggal: tanggalAkhir, keterangan: 'Tutup Prive ke Modal', akunDebit: '3-1000', akunKredit: '3-2000', debit: priveBal, kredit: priveBal, isJurnalPenutup: true, createdAt: new Date().toISOString() });
            }
    ...
    simpanSaldoAwal: function() {
        ...
        this.dataSaldoAwal.forEach(function(sa) {
            if (sa.periode === periode) batch.delete(window.sb.from('saldoAwal').doc(sa.id));
        });
        ...
        for (var k in this.COA) {
            ...
            if (nominal > 0) {
                var ref = window.sb.from('saldoAwal').doc();
