...
window.AppKeuanganAkuntansi = {
    ...
    render: function() {
        ...
        var tabs = [
            ...
            {id:'neraca', label:'Neraca'}, {id:'penutup', label:'Jurnal Penutup'}, {id:'saldo_awal', label:'Saldo Awal'}
        ];
    ...
    init: function() {
        ...
        var pTrx = window.sb.from('transaksi').select('*').gte('tanggal', startDate).lte('tanggal', endDate);
        var pKasKeluar = window.sb.from('kas_keluar').select('*').eq('status', 'approved').gte('tanggal', startDate).lte('tanggal', endDate);
        var pBeliStok = window.sb.from('pembelianStok').select('*').gte('tanggal', startDate).lte('tanggal', endDate);
        var pGaji = window.sb.from('penggajian').select('*').eq('bulan', bulan);
        var pJurnalManual = window.sb.from('jurnal_manual').select('*').gte('tanggal', startDate).lte('tanggal', endDate);
        var pSaldoAwal = window.sb.from('saldo_awal').select('*').lte('periode', bulan);
        Promise.all([pTrx, pKasKeluar, pBeliStok, pGaji, pJurnalManual, pSaldoAwal]).then(function(results) {
            ...
            results[0].data.forEach(function(doc) {
                ...
                var pendapatanJasa = (t.total_tindakan || 0) + (t.total_racik || 0) + (t.jasa_resep || 0);
                var kasBank = t.metode_bayar === 'cash' ? '1-1100' : '1-1200';

                self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'Penjualan ' + t.tipe, akun_debit: kasBank, akun_kredit: '', debit: t.total_akhir || 0, kredit: 0, isManual: false, tipeJurnal: 'Otomatis' });
                if (omzetNonPPN > 0) self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'Penjualan Obat Non-PPN', akun_debit: '', akun_kredit: '4-1100', debit: 0, kredit: omzetNonPPN, isManual: false, tipeJurnal: 'Otomatis' });
                if (omzetPPNNeto > 0) self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'Penjualan Obat PPN', akun_debit: '', akun_kredit: '4-1200', debit: 0, kredit: omzetPPNNeto, isManual: false, tipeJurnal: 'Otomatis' });
                if (ppnKeluaran > 0) self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'PPN Keluaran', akun_debit: '', akun_kredit: '2-1200', debit: 0, kredit: ppnKeluaran, isManual: false, tipeJurnal: 'Otomatis' });
                if (pendapatanJasa > 0) {
                    self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'Pendapatan Tindakan Medis', akun_debit: '', akun_kredit: '4-1300', debit: 0, kredit: t.total_tindakan || 0, isManual: false, tipeJurnal: 'Otomatis' });
                    self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'Pendapatan Jasa Resep/Racik', akun_debit: '', akun_kredit: '4-1400', debit: 0, kredit: (t.total_racik || 0) + (t.jasa_resep || 0), isManual: false, tipeJurnal: 'Otomatis' });
                }
                ...
                var selisih = (t.total_akhir || 0) - totalKreditSementara;
                if (Math.abs(selisih) > 0) {
                    if (selisih > 0) self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'Pendapatan Pembulatan/Lain', akun_debit: '', akun_kredit: '4-1500', debit: 0, kredit: selisih, isManual: false, tipeJurnal: 'Otomatis' });
                    else self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'Beban Selisih Kas/Lain', akun_debit: '5-3000', akun_kredit: '', debit: Math.abs(selisih), kredit: 0, isManual: false, tipeJurnal: 'Otomatis' });
                }

                // Jurnal HPP
                if (hppNonPPN > 0) {
                    self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'HPP Obat Non-PPN', akun_debit: '5-1100', akun_kredit: '', debit: hppNonPPN, kredit: 0, isManual: false, tipeJurnal: 'Otomatis' });
                    self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'Kredit Persediaan Non-PPN', akun_debit: '', akun_kredit: '1-1400', debit: 0, kredit: hppNonPPN, isManual: false, tipeJurnal: 'Otomatis' });
                }
                if (hppPPN > 0) {
                    self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'HPP Obat PPN', akun_debit: '5-1200', akun_kredit: '', debit: hppPPN, kredit: 0, isManual: false, tipeJurnal: 'Otomatis' });
                    self.dataJurnal.push({ tanggal: t.tanggal, keterangan: 'Kredit Persediaan PPN', akun_debit: '', akun_kredit: '1-1410', debit: 0, kredit: hppPPN, isManual: false, tipeJurnal: 'Otomatis' });
                }
            });

            // Jurnal Otomatis Pembelian Stok
            results[2].data.forEach(function(doc) {
                var b = doc;
                ...
                if (valNonPPN > 0) self.dataJurnal.push({ tanggal: b.tanggal, keterangan: 'Beli Stok Non-PPN', akun_debit: '1-1400', akun_kredit: '', debit: valNonPPN, kredit: 0, isManual: false, tipeJurnal: 'Otomatis' });
                if (valPersediaanPPN > 0) self.dataJurnal.push({ tanggal: b.tanggal, keterangan: 'Beli Stok PPN', akun_debit: '1-1410', akun_kredit: '', debit: valPersediaanPPN, kredit: 0, isManual: false, tipeJurnal: 'Otomatis' });
                if (ppnMasukan > 0) self.dataJurnal.push({ tanggal: b.tanggal, keterangan: 'PPN Masukan', akun_debit: '2-1200', akun_kredit: '', debit: ppnMasukan, kredit: 0, isManual: false, tipeJurnal: 'Otomatis' });

                // FIX: koleksi pembelian memakai field `metode_pembayaran` ('tunai'/'kredit') & `total_harga`,
                //      bukan `metode_bayar`/`totalTagihan`. Sebelumnya semua pembelian salah diakui sbg Bank.
                var akun_kredit = b.metode_pembayaran === 'kredit' ? '2-1100' : (b.metode_pembayaran === 'tunai' ? '1-1100' : '1-1200');
                self.dataJurnal.push({ tanggal: b.tanggal, keterangan: 'Pembayaran ke Supplier', akun_debit: '', akun_kredit: akun_kredit, debit: 0, kredit: b.total_harga || 0, isManual: false, tipeJurnal: 'Otomatis' });
            });

            // Jurnal Otomatis Pengeluaran Kas
            results[1].data.forEach(function(doc) {
                var p = doc;
                
                // FIX #5: Skip gaji & tunjangan di kas_keluar agar tidak double dengan penggajian
                if (p.kategori === 'gaji' || p.kategori === 'tunjangan') return; 
                
                var akunBeban = '5-2300'; // Default Operasional
                self.dataJurnal.push({ tanggal: p.tanggal, keterangan: p.keterangan, akun_debit: akunBeban, akun_kredit: '', debit: p.jumlah || 0, kredit: 0, isManual: false, tipeJurnal: 'Otomatis' });
                self.dataJurnal.push({ tanggal: p.tanggal, keterangan: 'Kas Keluar', akun_debit: '', akun_kredit: '1-1100', debit: 0, kredit: p.jumlah || 0, isManual: false, tipeJurnal: 'Otomatis' });
            });

            // Jurnal Otomatis Penggajian
            results[3].data.forEach(function(doc) {
                var g = doc;
                var total_gaji = 0, totalTunjangan = 0;
                if (g.dataPenggajian) {
                    g.dataPenggajian.forEach(function(dp) {
                        total_gaji += dp.total_gaji || 0;
                        totalTunjangan += dp.totalTunjangan || 0;
                    });
                }
                if(total_gaji > 0) self.dataJurnal.push({ tanggal: g.tanggal, keterangan: 'Pembayaran Gaji', akun_debit: '5-2100', akun_kredit: '', debit: total_gaji, kredit: 0, isManual: false, tipeJurnal: 'Otomatis' });
                if(totalTunjangan > 0) self.dataJurnal.push({ tanggal: g.tanggal, keterangan: 'Pembayaran Tunjangan', akun_debit: '5-2200', akun_kredit: '', debit: totalTunjangan, kredit: 0, isManual: false, tipeJurnal: 'Otomatis' });
                if(total_gaji > 0 || totalTunjangan > 0) self.dataJurnal.push({ tanggal: g.tanggal, keterangan: 'Kas Keluar Payroll', akun_debit: '', akun_kredit: '1-1100', debit: 0, kredit: total_gaji + totalTunjangan, isManual: false, tipeJurnal: 'Otomatis' });
            });

            // Jurnal Manual
            results[4].data.forEach(function(doc) {
                var j = doc; j.id = doc.id; j.isManual = true; j.tipeJurnal = j.isJurnalPenutup ? 'Penutup' : 'Manual';
    ...
    switchTab: function(tab) {
        var tabs = ['jurnal', 'bukuBesar', 'neracaSaldo', 'labaRugi', 'neraca', 'penutup', 'saldo_awal'];
        var self = this;
        tabs.forEach(function(t) {
        ...
        else if (tab === 'saldo_awal') this.renderSaldoAwal();
    },
    ...
    renderJurnalUmum: function() {
        ...
        sortedJurnal.forEach(function(j) {
            var namaDebit = j.akun_debit ? (AppKeuanganAkuntansi.COA[j.akun_debit] ? AppKeuanganAkuntansi.COA[j.akun_debit].nama : j.akun_debit) : '';
            var namaKredit = j.akun_kredit ? (AppKeuanganAkuntansi.COA[j.akun_kredit] ? AppKeuanganAkuntansi.COA[j.akun_kredit].nama : j.akun_kredit) : '';
    ...
    simpanJurnal: function() {
        var obj = {
            ...
            akun_debit: document.getElementById('jr-debit').value,
            akun_kredit: document.getElementById('jr-kredit').value,
        ...
        if (obj.debit <= 0 || obj.akun_debit === obj.akun_kredit) { Utils.toast('Input tidak valid', 'error'); return; }

        window.sb.from('jurnal_manual').insert(obj).then(function() {
    ...
    hapusJurnal: function(id) {
        ...
        window.sb.from('jurnal_manual').delete().eq('id', id).then(function() {
    ...
    renderBukuBesarDetail: function() {
        ...
        this.dataJurnal.filter(function(j) { return j.akun_debit === akunId || j.akun_kredit === akunId; }).sort(function(a,b){return (a.tanggal||'').localeCompare(b.tanggal||'');}).forEach(function(j) {
            var debit = j.akun_debit === akunId ? (j.debit || 0) : 0;
            var kredit = j.akun_kredit === akunId ? (j.kredit || 0) : 0;
            saldo += (debit - kredit);
    ...
    renderLabaRugi: function() {
        ...
        var jasa_resep = saldos['4-1400'].kredit;
        var pendLain = saldos['4-1500'].kredit;
        
        var totalPendapatan = penjNonPPN + penjPPN + jasaMedis + jasa_resep + pendLain;
        ...
        html += '<tr><td class="py-1 pl-4">Pendapatan Jasa Resep / Racik</td><td class="text-right">' + Utils.formatRupiah(jasa_resep) + '</td></tr>';
    ...
    postingJurnalPenutup: function() {
        ...
        window.sb.from('jurnal_manual').select('*').eq('isJurnalPenutup', true).gte('tanggal', bulan + '-01').lte('tanggal', bulan + '-31').then(function(snap) { if(snap.error) throw snap.error;
            ...
            // TODO: batch Firestore → gunakan Promise.all di Supabase
        var _batchOps = [];
            var parts = bulan.split('-');
            ...
            for (var k in self.COA) {
                if (self.COA[k].kategori === 'Pendapatan') {
                    ...
                    if (balK > 0) {
                        ...
                        var ref1 = window.sb.from('jurnal_manual').select('*').doc();
                        batch.set(ref1, { tanggal: tanggalAkhir, keterangan: 'Tutup Pendapatan ' + self.COA[k].nama, akun_debit: k, akun_kredit: '3-9000', debit: balK, kredit: balK, isJurnalPenutup: true, createdAt: new Date().toISOString() });
                    }
                }
            }
            ...
            for (var k2 in self.COA) {
                if (self.COA[k2].kategori === 'Beban') {
                    ...
                    if (balD > 0) {
                        totalBeban += balD;
                        var ref2 = window.sb.from('jurnal_manual').select('*').doc();
                        batch.set(ref2, { tanggal: tanggalAkhir, keterangan: 'Tutup Beban ' + self.COA[k2].nama, akun_debit: '3-9000', akun_kredit: k2, debit: balD, kredit: balD, isJurnalPenutup: true, createdAt: new Date().toISOString() });
                    }
                }
            }
            ...
            if (labaBersih !== 0) {
                var ref3 = window.sb.from('jurnal_manual').select('*').doc();
                if (labaBersih > 0) { // Untung
                    batch.set(ref3, { tanggal: tanggalAkhir, keterangan: 'Tutup Ikhtisar L/R (Laba)', akun_debit: '3-9000', akun_kredit: '3-3000', debit: labaBersih, kredit: labaBersih, isJurnalPenutup: true, createdAt: new Date().toISOString() });
                } else { // Rugi
                    batch.set(ref3, { tanggal: tanggalAkhir, keterangan: 'Tutup Ikhtisar L/R (Rugi)', akun_debit: '3-3000', akun_kredit: '3-9000', debit: Math.abs(labaBersih), kredit: Math.abs(labaBersih), isJurnalPenutup: true, createdAt: new Date().toISOString() });
                }
            }
            ...
            if (priveBal > 0) {
                var ref4 = window.sb.from('jurnal_manual').select('*').doc();
                batch.set(ref4, { tanggal: tanggalAkhir, keterangan: 'Tutup Prive ke Modal', akun_debit: '3-1000', akun_kredit: '3-2000', debit: priveBal, kredit: priveBal, isJurnalPenutup: true, createdAt: new Date().toISOString() });
            }
    ...
    simpanSaldoAwal: function() {
        ...
        this.dataSaldoAwal.forEach(function(sa) {
            if (sa.periode === periode) batch.delete(window.sb.from('saldo_awal').doc(sa.id));
        });
        ...
        for (var k in this.COA) {
            ...
            if (nominal > 0) {
                var ref = window.sb.from('saldo_awal').select('*').doc();
                batch.set(ref, { akunId: k, nominal: nominal, tipe: tipe, periode: periode, createdAt: new Date().toISOString() });
            }
        }
    ...
    hitungSaldoAkun: function() {
        ...
        this.dataJurnal.forEach(function(j) {
            if (j.akun_debit && saldos[j.akun_debit]) saldos[j.akun_debit].debit += (j.debit || 0);
            if (j.akun_kredit && saldos[j.akun_kredit]) saldos[j.akun_kredit].kredit += (j.kredit || 0);
        });
        return saldos;
    },
