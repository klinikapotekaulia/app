...
window.AppKeuanganLaporanKeuangan = {
    ...
    init: function() {
        ...
        var pKeluar = window.sb.from('kas_keluar').eq('status', 'approved').gte('tanggal', startDate).lte('tanggal', endDate).get();
        var pBeli = window.sb.from('pembelian').gte('tanggal', startDate).lte('tanggal', endDate).get();

        Promise.all([pTrx, pKeluar, pBeli]).then(function(results) {
            self.dataTransaksi = [];
            results[0].forEach(function(doc) { var d = doc; d.id = doc.id; self.dataTransaksi.push(d); });

            self.dataPengeluaran = [];
            results[1].forEach(function(doc) { var d = doc; d.id = doc.id; self.dataPengeluaran.push(d); });

            self.dataPembelian = [];
            results[2].forEach(function(doc) { var d = doc; d.id = doc.id; self.dataPembelian.push(d); });
