...
window.AppKeuanganLaporanKeuangan = {
    ...
    init: function() {
        ...
        var pKeluar = window.sb.from('kas_keluar').select('*').eq('status', 'approved').gte('tanggal', startDate).lte('tanggal', endDate);
        var pBeli = window.sb.from('pembelian').select('*').gte('tanggal', startDate).lte('tanggal', endDate);
        Promise.all([pTrx, pKeluar, pBeli]).then(function(results) {
            self.dataTransaksi = [];
            results[0].data.forEach(function(doc) { var d = doc; d.id = doc.id; self.dataTransaksi.push(d); });

            self.dataPengeluaran = [];
            results[1].data.forEach(function(doc) { var d = doc; d.id = doc.id; self.dataPengeluaran.push(d); });

            self.dataPembelian = [];
            results[2].data.forEach(function(doc) { var d = doc; d.id = doc.id; self.dataPembelian.push(d); });
    ...
    renderReport: function() {
        ...
        var total_tindakan = 0, total_racik = 0, totalJasaResep = 0;
        ...
        this.dataTransaksi.forEach(function(t) {
            ...
            totalOmzet += omzetObat + (t.total_racik || 0) + (t.total_tindakan || 0) + (t.jasa_resep || 0);
            totalHPP += hppObat;
            total_tindakan += (t.total_tindakan || 0);
            total_racik += (t.total_racik || 0);
            totalJasaResep += (t.jasa_resep || 0);
            totalLabaKotor += (omzetObat - hppObat) + (t.total_racik || 0) + (t.total_tindakan || 0) + (t.jasa_resep || 0);

            if (t.metode_bayar === 'cash') cashMasuk += t.total_akhir || 0;
            else if (t.metode_bayar === 'transfer') transferMasuk += t.total_akhir || 0;
            else if (t.metode_bayar === 'qris') qrisMasuk += t.total_akhir || 0;
        });
        ...
        this.dataPembelian.forEach(function(b) {
            if (b.metode_pembayaran === 'tunai') totalBeliTunai += b.total_harga || 0;
            else totalBeliKredit += b.total_harga || 0;
        });
        ...
        html += '    <div class="flex justify-between pl-4 text-xs"><span class="text-slate-400">+ Tindakan & Jasa</span><span class="text-emerald-500">' + Utils.formatRupiah(total_tindakan + totalJasaResep) + '</span></div>';
        html += '    <div class="flex justify-between pl-4 text-xs"><span class="text-slate-400">+ Racik & Lainnya</span><span class="text-emerald-500">' + Utils.formatRupiah(total_racik) + '</span></div>';
        ...
        if (this.dataTransaksi.length === 0) {
            ...
            this.dataTransaksi.forEach(function(t) {
                ...
                html += '<td class="px-4 py-3 font-medium text-gray-800 dark:text-white">' + Utils.escapeHtml(t.nama_pasien || 'Bebas') + '</td>';
                ...
                html += '<td class="px-4 py-3 text-xs uppercase">' + Utils.escapeHtml(t.metode_bayar || '-') + '</td>';
                html += '<td class="px-4 py-3 text-right font-semibold text-gray-800 dark:text-white">' + Utils.formatRupiah(t.total_akhir) + '</td>';
                html += '</tr>';
            });
        }
        html += '      </tbody></table>';
