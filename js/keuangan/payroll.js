...
window.AppKeuanganPayroll = {
    ...
    init: function() {
        ...
        var pCfgGaji = window.sb.from('pengaturanGaji').select('*').eq('id', 'global').single();
        var pCfgBagi = window.sb.from('pengaturanPembagian').select('*').eq('id', 'global').single();
    ...
    simpanPayroll: function() {
        ...
        window.sb.from('payrollHistory').eq('bulan', bulan).limit(1).get().then(function(snap) {
            if (!snap.empty) {
            ...
            var ref = window.sb.from('payrollHistory').doc();
            batch.set(ref, {
                bulan: bulan,
