...
window.AppKeuanganPayroll = {
    ...
    init: function() {
        ...
        var pAbsen = window.sb.from('absensi').select('*').gte('tanggal', startDate).lte('tanggal', endDate);
        ...
        Promise.all([pKary, pTrx, pAbsen, pCfgGaji, pCfgBagi]).then(function(results) {
            self.dataKaryawan = [];
            results[0].data.forEach(function(doc) { var d = doc; d.id = doc.id; self.dataKaryawan.push(d); });

            self.dataTransaksi = [];
            results[1].data.forEach(function(doc) { var d = doc; d.id = doc.id; self.dataTransaksi.push(d); });

            self.dataAbsensi = [];
            results[2].data.forEach(function(doc) { self.dataAbsensi.push(doc); });
    ...
    simpanPayroll: function() {
        ...
        window.sb.from('payroll_history').select('*').eq('bulan', bulan).limit(1).then(function(snap) { if(snap.error) throw snap.error;
        ...
        var _batchOps = [];
            ...
            var ref = window.sb.from('payroll_history').select('*').doc();
            batch.set(ref, {
                bulan: bulan,
