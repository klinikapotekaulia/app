...
window.AppDashboard = {
    ...
    renderKlinik: function() {
        ...
        var pResep = window.sb.from('rekam_medis').eq('status', 'selesai').get();
    ...
    renderApotek: function() {
        var self = this;
        var today = new Date().toISOString().split('T')[0];
        var pTrx = window.sb.from('transaksi').eq('tanggal', today).get();
        var pObat = window.sb.from('obat').get();
        var pResep = window.sb.from('rekam_medis').eq('status', 'selesai').get();
    ...
    renderAdmin: function() {
        var self = this;
        var today = new Date().toISOString().split('T')[0];
        var pAbsen = window.sb.from('absensi').eq('tanggal', today).get();
        var pKary = window.sb.from('karyawan').eq('status', 'aktif').get();
        var pSO = window.sb.from('stock_opname_requests').eq('status', 'pending').get();
        var pKas = window.sb.from('kas_keluar').eq('status', 'pending').get();
    ...
    renderKeuangan: function() {
        ...
        var pTrx = window.sb.from('transaksi').gte('tanggal', startMonth).lte('tanggal', today).get();
        var pKasKeluar = window.sb.from('kas_keluar').eq('status', 'approved').gte('tanggal', startMonth).lte('tanggal', today).get();
        var pBeli = window.sb.from('pembelian').eq('metode_pembayaran', 'tunai').gte('tanggal', startMonth).lte('tanggal', today).get();
        ...
        var pHutangJatuhTempo = window.sb.from('pembelian').in('status_pelunasan', ['belum_lunas','sebagian','belum']).get();
        ...
        Promise.all([pTrx, pKasKeluar, pBeli, pHutangJatuhTempo]).then(function(results) {
            ...
            var bebanOp = 0;
            results[1].forEach(function(doc) { bebanOp += doc.jumlah || 0; });

            var beliTunai = 0;
            results[2].forEach(function(doc) { beliTunai += doc.total_harga || 0; });
