...
window.AppDashboard = {
    ...
    renderKlinik: function() {
        ...
        var pResep = window.sb.from('rekamMedis').eq('status', 'selesai').get();
    ...
    renderAdmin: function() {
        ...
        var pSO = window.sb.from('stockOpnameRequests').eq('status', 'pending').get();
        var pKas = window.sb.from('kasKeluar').eq('status', 'pending').get();
    ...
    renderKeuangan: function() {
        ...
        var pKasKeluar = window.sb.from('kasKeluar').eq('status', 'approved').gte('tanggal', startMonth).lte('tanggal', today).get();
        var pBeli = window.sb.from('pembelian').eq('metodePembayaran', 'tunai').gte('tanggal', startMonth).lte('tanggal', today).get();
        ...
        var pHutangJatuhTempo = window.sb.from('pembelian').in('statusPelunasan', ['belum_lunas','sebagian','belum']).get();
        ...
        Promise.all([pTrx, pKasKeluar, pBeli, pHutangJatuhTempo]).then(function(results) {
            ...
            var beliTunai = 0;
            results[2].forEach(function(doc) { beliTunai += doc.totalHarga || 0; });
