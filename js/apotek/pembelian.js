...
window.AppApotekPembelian = {
    ...
    init: function() {
        window.sb.from('obat').select('*').then(function(snap) { if(snap.error) throw snap.error;
            AppApotekPembelian.obatList = [];
            (snap.data || []).forEach(function(doc) { var d = doc; d.id = doc.id; AppApotekPembelian.obatList.push(d); });
    ...
    addItem: function() {
        ...
        this.obatList.forEach(function(o) {
            html += '<option value="' + o.id + '">' + Utils.escapeHtml(o.nama_obat) + ' (Stok: ' + (o.stok || 0) + ')</option>';
        });
        html += '</select></div>';
    ...
    simpan: function() {
        var no_faktur = document.getElementById('beli-nofaktur').value.trim();
        var tgl = document.getElementById('beli-tgl').value;
        var supplier = document.getElementById('beli-supplier').value.trim();
        var metode = document.getElementById('beli-metode').value;
        var jatuh_tempo = document.getElementById('beli-jatuh-tempo').value;

        if (!no_faktur || !tgl || !supplier || !metode) {
            Utils.toast('Lengkapi semua data header', 'error');
            return;
        }
        if (metode === 'kredit' && !jatuh_tempo) {
        ...
        var _batchOps = [];
        var pRef = window.sb.from('pembelian').select('*').doc();
        batch.set(pRef, {
            no_faktur: no_faktur,
            tanggal: tgl,
            supplier: supplier,
            metode_pembayaran: metode,
            jatuh_tempo: metode === 'kredit' ? jatuh_tempo : null,
        ...
        items.forEach(function(item) {
            if (item.qty > 0) {
                var obatRef = window.sb.from('obat').select('*').doc(item.obat_id);
    ...
    loadRiwayat: function() {
        ...
        window.sb.from('pembelian').select('*').order('createdAt', { ascending: false }).limit(10).then(function(snap) { if(snap.error) throw snap.error;
            var riwayat = [];
            (snap.data || []).forEach(function(doc) { var d = doc; d.id = doc.id; riwayat.push(d); });
            ...
            riwayat.forEach(function(r) {
                ...
                html += '<td class="px-3 py-2 font-mono">' + Utils.escapeHtml(r.no_faktur) + '</td>';
                html += '<td class="px-3 py-2">' + metodeBadge + '</td>';
                html += '<td class="px-3 py-2">' + statusBadge + '</td>';
                html += '<td class="px-3 py-2 text-right font-semibold text-primary-600">' + Utils.formatRupiah(r.total_harga) + '</td>';
                html += '</tr>';
            });
