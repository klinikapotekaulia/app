...
window.AppApotekRetur = {
    ...
    init: function() {
        ...
        Promise.all([
            ...
            self.masterObat = [];
            results[0].data.forEach(function(doc) { var d = doc; d.id = doc.id; self.masterObat.push(d); });
    ...
    konfirmasi: function(id) {
        ...
        var _batchOps = [];
        var returRef = window.sb.from('retur').select('*').doc(id);
        var obatRef  = window.sb.from('obat').select('*').doc(retur.obat_id);
