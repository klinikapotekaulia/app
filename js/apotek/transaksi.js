...
window.AppApotekTransaksi = {
    ...
    init: function() {
        ...
        Promise.all([
            ...
            window.sb.from('pengaturanPembagian').select('*').eq('id', 'global').single(),
            window.sb.from('rekamMedis').eq('status', 'selesai').get(),
            window.sb.from('antrian').eq('tanggal', today).get(),
            window.sb.from('masterTindakan').eq('aktif', true).get()
            ...
            results[4].forEach(function(doc) {
                var d = doc; d.id = doc.id; self.masterTindakan.push(d);
            });
    ...
    simpan: function() {
        ...
        items.forEach(function(item) {
            var oref = window.sb.from('obat').doc(item.obatId);
            batch.update(oref, { stok: /* TODO_INCREMENT(-item.jumlah) */ });
        });
        if (self.tipe === 'resep_klinik' && obj.resepId) {
            batch.update(window.sb.from('rekamMedis').doc(obj.resepId), { statusResep: 'selesai' });
        }
