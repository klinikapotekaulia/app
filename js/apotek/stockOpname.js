...
window.AppApotekStockOpname = {
    ...
    loadMasterObat: function() {
        var self = this;
        window.sb.from('obat').order('namaObat', { ascending: true }).get().then(snap => {
            self.data = [];
            (data || []).forEach(doc => { 
                var d = doc; d.id = doc.id; d.stokFisik = ''; 
                self.data.push(d); 
            });
    ...
    ajukanOpname: function() {
        ...
        window.sb.from('stockOpnameRequests').insert({
    ...
    loadRequests: function() {
        ...
        window.sb.from('stockOpnameRequests').eq('status', 'pending').get().then(snap => {
            self.requests = [];
    ...
    approveReq: function(reqId) {
        ...
        req.items.forEach(function(it) {
            var ref = window.sb.from('obat').doc(it.obatId);
        ...
        var reqRef = window.sb.from('stockOpnameRequests').doc(reqId);
        batch.update(reqRef, { 
            status: 'approved', 
        ...
        // 3. Catat ke history
        var histRef = window.sb.from('stockOpnameHistory').doc();
        batch.set(histRef, {
    ...
    rejectReq: function(reqId) {
        ...
        window.sb.from('stockOpnameRequests').update({ 
            status: 'rejected',
            approvedBy: window.currentUserName || 'Admin',
            approvedAt: new Date().toISOString()
        }).then(function() {
