...
window.AppApotekStockOpname = {
    ...
    loadMasterObat: function() {
        var self = this;
        window.sb.from('obat').select('*').order('nama_obat', { ascending: true }).then(function(snap) { if(snap.error) throw snap.error;
            self.data = [];
    ...
    loadRequests: function() {
        ...
        window.sb.from('stock_opname_requests').select('*').eq('status', 'pending').then(function(snap) { if(snap.error) throw snap.error;
            self.requests = [];
            (snap.data || []).forEach(function(d) { self.requests.push(d); });
    ...
    approveReq: function(reqId) {
        ...
        req.items.forEach(function(it) {
            var ref = window.sb.from('obat').select('*').doc(it.obat_id);
        ...
        var reqRef = window.sb.from('stock_opname_requests').select('*').doc(reqId);
        batch.update(reqRef, { 
            status: 'approved', 
        ...
        // 3. Catat ke history
        var histRef = window.sb.from('stock_opname_history').select('*').doc();
        batch.set(histRef, {
